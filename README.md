...
Initialization: Start from a random configuration for the $L \times L$ lattice, where each spin is randomly assigned a value of $+1$ or $-1$.

Proposal: Propose a new energy configuration by picking a random spin and flipping its sign (single spin-flip).

Energy Evaluation: Compute the energy difference between the new and old configurations:
...
Acceptance Step: According to the Metropolis criterion, the probability of accepting the proposed configuration is given by:
...
Iteration: Repeat these steps to evolve the system over time.
